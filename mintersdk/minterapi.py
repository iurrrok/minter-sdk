...
class MinterAPI(object):
    ...
    def get_nonce(self, address):
        ...
        nonce = int(balance['transaction_count']) + 1
    ...
    def _request(self, command, request_type='get', **kwargs):
        ...
        # Trying make request
        try:
            value = list(kwargs['params'].values())[0]
            kwargs['params'].pop(next(iter(kwargs['params'])))
            if not isinstance(value, str):
                value = self._unpack_value(value)
            url = self.api_url + command + '/' + value
            if request_type == 'get':
                response = requests.get(url, params=kwargs['params'])
            elif request_type == 'post':
                response = requests.post(url, params=kwargs['params'])
            else:
                response = None
    ...
    def _unpack_value(self, item):
        if isinstance(item, str):
            return item
        item = item[0]
        return self._unpack_value(item)
